    ...
    while (control.isRunning) {
        try {
            ...
            for (let i = 0; i < marketContexts.length; i++) {
                ...
                const instrSet = makeMarketUpdateInstructions(entropyGroup, state.cache, entropyAccount, marketContexts[i], ftxBook, ftxFundingRate, IVFundingOffset);
...
function makeMarketUpdateInstructions(group, cache, entropyAccount, marketContext, ftxBook, ftxFundingRate, IVFundingOffset) {
    ...
    let bidPrice = fairValue * (1 - edge + lean + bias + 0.1 * IVFundingOffset);
